...
import { requiresApproval, type GateAction } from "../riskGate";

const click = (targetText: string, targetRole = "button"): GateAction => ({
  kind: "click",
  targetText,
  targetRole,
});
...
describe("RiskGate", () => {
  ...
  it("lets a plainly safe action proceed without a gate", () => {
    ...
    expect(
      requiresApproval({ kind: "scroll", targetText: "", targetRole: "" }),
    ).toBe(false);
  });
  ...
  it("does not gate a 'submit'-labeled link (navigation, not a form submission)", () => {
    ...
    expect(requiresApproval(click("How to submit a request", "link"))).toBe(
      false,
    );
  });
