...
describe("NarrationDirector", () => {
  it("choreographs a click as cursor travel, highlight dwell, then ripple", () => {
    const cmds = narrate({ kind: "click", label: "the Sign in button" }, onScreen);

    expect(cmds.map((c) => c.type)).toEqual(["cursor-move", "highlight", "ripple"]);
  ...
  it("types character-by-character after travelling to the field, with no ripple", () => {
    const cmds = narrate({ kind: "type", label: "the Email field" }, onScreen, "hi");
